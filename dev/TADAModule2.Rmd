...
```{r install_remotes, results = 'hide', eval = T}
...
(or dataframes if `fill_USGS_catch = TRUE`, see section **Filling in
...
`TADA_CreateATTAINSAUMLCrosswalk()` will use the NHD HiRes
(`resolution = "Hi"`) for filling in missing ATTAINS catchments.
However, the user can choose to change the resolution to the NHDPlus V2
by setting `resolution = "Med".`
...
`TADA_ViewATTAINS()` also mapped the `without_ATTAINS_catchments`. These
are represented as clear polygons with black outlines.
