...
mod_map_table_selector_ui <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(column(
      width = 12,
      htmltools::h4("Choose sites to review the results"),
      htmltools::p(
        "Select the sites on the map or in the table to view the results."
      ),
      htmltools::div(
        style = "margin-bottom: 30px;",
        shinyjs::disabled(
          shiny::actionButton(
            inputId = ns("select_all_sites"),
            label = "Select All Sites",
            shiny::icon("circle-check"),
            style = "color: #fff; background-color: #337ab7; border-color: #2e6da4"
          ),
          shiny::actionButton(
            inputId = ns("deselect_all_sites"),
            label = "Deselect All Sites",
            shiny::icon("circle"),
            style = "color: #fff; background-color: #337ab7; border-color: #2e6da4"
          )
        )
    ...
    fluidRow(
      column(width = 6, leaflet::leafletOutput(ns("map_selector"))),
      column(width = 6, DT::DTOutput(ns("table_selector")))
    )
  )
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    shiny::observe({
      ...
      output$map_selector <- leaflet::renderLeaflet({
        req(is.data.frame(tadat$site_AU_table))
        req(nrow(tadat$site_AU_table) > 0)
      ...
      # Proxy
      map_selector_proxy <- leaflet::leafletProxy(
        "map_selector",
        session = session
      )
      table_selector_proxy <- DT::dataTableProxy(
        "table_selector",
        session = session
      )
      ...
      shiny::observe({
        shinyjs::toggleState(
          id = "select_all_sites",
          condition = !is.null(tadat$site_AU_table)
        )
        shinyjs::toggleState(
          id = "deselect_all_sites",
          condition = !is.null(tadat$site_AU_table)
        )
      })
      ...
      # Select all sites
      observeEvent(
        input$select_all_sites,
        {
          req(tadat$site_AU_table)

          # Select all rows
          all_rows <- seq_len(nrow(tadat$site_AU_table))
          selected_idx(all_rows)
          table_selector_proxy |> DT::selectRows(all_rows)

          # Highlight all points on map
          map_selector_proxy |> leaflet::clearGroup("highlighted_point")
          map_selector_proxy |>
            leaflet::addCircleMarkers(
              lng = tadat$site_AU_table$TADA.LongitudeMeasure,
              lat = tadat$site_AU_table$TADA.LatitudeMeasure,
              layerId = paste0(
                tadat$site_AU_table$TADA.MonitoringLocationIdentifier,
                "_new"
              ),
              radius = 9,
              stroke = TRUE,
      ...
      observeEvent(
        input$table_selector_rows_selected,
        ignoreInit = TRUE,
        ignoreNULL = FALSE,
        {
          req(is.data.frame(tadat$site_AU_table))
            ...
            if (nrow(sel) > 0) {
              map_selector_proxy |>
                leaflet::addCircleMarkers(
                  ...
                  layerId = paste0(
                    sel$TADA.MonitoringLocationIdentifier,
                    "_new"
                  ),
                  radius = 9,
                  stroke = TRUE,
      ...
      observeEvent(
        input$map_selector_marker_click,
        {
          req(is.data.frame(tadat$site_AU_table))
          req(nrow(tadat$site_AU_table) > 0)
      ...
      observeEvent(
        c(tadat$criteria_state_tribe, tadat$uses_select_re),
        {
    ...
    selected_monitoring_locations <- reactive({
      ...
      selected_rows <- tadat$site_AU_table |> dplyr::slice(idx)
