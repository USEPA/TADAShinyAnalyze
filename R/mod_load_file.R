  ...
  shiny::tagList(
    ...
    shiny::fluidRow(
      ...
      shiny::column(
        ...
        htmltools::p(
          "This app is used to analyze water quality data based on the criteria table (Step 2) through 
        either a batch analysis (Step 3) or custom analysis (Step 4)."
        ),

        htmltools::p(
          "This tab is used to upload three data files: 
        ...
        htmltools::p(
          "Second, upload a ML to AU crosswalk table (Step 1b, optional)."
        ),
        htmltools::p(
          "Third, upload an AU to Designated Uses crosswalk table (Step 1c, optionl).
        The files for Steps 1b and 1c can be supplied from an organization 
        or generated using the",
          htmltools::a(
            "TADAShinyJoinToAU App.",
            href = "https://tetratech-wtr-wne.shinyapps.io/TADAShinyJoinToAU/",
            target = "_blank"
          )
        ),
        htmltools::p(
          "Users need to at least upload the water 
      ...
      shiny::column(
        ...
        shiny::verbatimTextOutput(
          outputId = ns("mlid_input_summary"),
          placeholder = TRUE
        ),
    ...
    shiny::fluidRow(
      ...
      shiny::column(
        ...
        htmltools::p("Summary of loaded file (blank until file is loaded)."),
        shiny::verbatimTextOutput(
          outputId = ns("mltoau_input_summary"),
          placeholder = TRUE
        ),
        htmltools::h3("Data Preview"),
        htmltools::p(
    ...
    shiny::fluidRow(
      ...
      shiny::column(
        ...
        htmltools::p("Summary of loaded file (blank until file is loaded)."),
        shiny::verbatimTextOutput(
          outputId = ns("autouse_input_summary"),
          placeholder = TRUE
        ),
        htmltools::h3("Data Preview"),
        htmltools::p(
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    df_mlid_input <- shiny::eventReactive(input$mlid_input_file, {
      ...
      message(paste0(
        ...
        input$mlid_input_file$name,
        "\n",
        "Monitoring Location Import, file path: ",
        file_path_mlid_input,
        "\n",
      ...
      if (file_ext_mlid_input %in% c("csv", "tsv", "txt")) {
        df_mlid_input <- utils::read.delim(
          file_path_mlid_input,
          header = TRUE,
        ...
        df_mlid_input <- readxl::read_excel(
          file_path_mlid_input,
          na = c("NA", ""),
          trim_ws = TRUE,
          col_names = TRUE,
    ...
    output$mlid_input_summary <- shiny::renderText({
      ...
      if (is.null(df_mlid_input)) {
        ...
        paste0(
          "Loaded dataset has ",
          nrow(df_mlid_summary),
          " rows and ",
          ncol(df_mlid_summary),
          " columns.\n",
          "There are ",
          length(unique(df_mlid_summary$TADA.MonitoringLocationIdentifier)),
    ...
    df_mltoau_input <- shiny::eventReactive(input$mltoau_input_file, {
      # validate file is selected
      shiny::validate(need(
        !is.null(input$mltoau_input_file),
        "No file selected."
      ))
      ...
      shiny::showNotification(
        paste0(
          "ML to AU Crosswalk Import, separator: '",
          input$mltoau_separator,
          "'\n",
          "ML to AU Crosswalk Import, file name: ",
          input$mltoau_input_file$name,
          "\n"
        ),
        type = "message",
      ...
      if (file_ext_mltoau %in% c("csv", "tsv", "txt")) {
        df_mltoau_input <- utils::read.delim(
          file_path_mltoau,
          header = TRUE,
        ...
        df_mltoau_input <- readxl::read_excel(
          file_path_mltoau,
          na = c("NA", ""),
          trim_ws = TRUE,
          col_names = TRUE,
      ...
      # define required columns
      # TODO need to check this is correct
      mltoau_required_cols <- c(
        "TADA.MonitoringLocationIdentifier",
        "ATTAINS.AssessmentUnitIdentifier"
      )

      # get missing columns
      mltoau_missing_cols <- setdiff(
        mltoau_required_cols,
        names(df_mltoau_input)
      )
    ...
    output$mltoau_input_summary <- shiny::renderText({
      ...
      if (is.null(df_mltoau_input)) {
        ...
        paste0(
          "Loaded dataset has ",
          nrow(df_mltoau_summary),
          " rows and ",
          ncol(df_mltoau_summary),
          " columns.\n",
          "There are ",
          length(unique(df_mltoau_summary$TADA.MonitoringLocationIdentifier)),
          " unique monitoring locations.",
          "There are ",
          length(unique(df_mltoau_summary$ATTAINS.AssessmentUnitIdentifier)),
    ...
    df_autouse_input <- shiny::eventReactive(input$autouse_input_file, {
      # validate file is selected
      shiny::validate(need(
        !is.null(input$autouse_input_file),
        "No file selected."
      ))
      ...
      shiny::showNotification(
        paste0(
          "AU to Use Crosswalk Import, separator: '",
          input$autouse_separator,
          "'\n",
          "AU to Use Crosswalk Import, file name: ",
          input$autouse_input_file$name,
          "\n"
        ),
        type = "message",
      ...
      if (file_ext_autouse %in% c("csv", "tsv", "txt")) {
        df_autouse_input <- utils::read.delim(
          file_path_autouse,
          header = TRUE,
        ...
        df_autouse_input <- readxl::read_excel(
          file_path_autouse,
          na = c("NA", ""),
          trim_ws = TRUE,
          col_names = TRUE,
      ...
      # define required columns
      # TODO need to check this is correct
      autouse_required_cols <- c(
        "ATTAINS.AssessmentUnitIdentifier",
        "ATTAINS.UseName"
      )

      # get missing columns
      autouse_missing_cols <- setdiff(
        autouse_required_cols,
        names(df_autouse_input)
      )
    ...
    output$autouse_input_summary <- shiny::renderText({
      ...
      if (is.null(df_autouse_input)) {
        ...
        paste0(
          "Loaded dataset has ",
          nrow(df_autouse_summary),
          " rows and ",
          ncol(df_autouse_summary),
          " columns.\n",
          "There are ",
          length(unique(df_autouse_summary$ATTAINS.AssessmentUnitIdentifier)),
          " unique assessment units.",
          "There are ",
          length(unique(df_autouse_summary$ATTAINS.UseName)),
          " unique use types."
        )
      }
    }) # end renderText
