  ...
  tagList(fluidRow(
    column(
      ...
      shiny::selectizeInput(
        ...
        selected = NULL,
        multiple = FALSE,
        options = list(placeholder = "Select a parameter", create = FALSE)
      )
    ),
    column(
      width = 4,
      ...
      shiny::selectizeInput(
        ...
        selected = NULL,
        multiple = FALSE,
        options = list(placeholder = "Select the fraction", create = FALSE)
      )
    ),
    column(
      width = 4,
      ...
      shiny::selectizeInput(
        ...
        selected = NULL,
        multiple = FALSE,
        options = list(placeholder = "Select the uses", create = FALSE)
      )
    ),
    fluidRow(
      column(
        ...
        shiny::selectizeInput(
          ...
          selected = NULL,
          multiple = TRUE,
          options = list(placeholder = "Select location", create = FALSE)
        )
      ),
      column(
        width = 4,
      ...
      column(
        ...
        shiny::selectizeInput(
          ...
          selected = NULL,
          multiple = FALSE,
          options = list(placeholder = "Select the season", create = FALSE)
        )
      )
    ),
    fluidRow(
...
#'
#' @noRd
mod_analysis_plots_server <- function(
  id,
  # Excursion dataset
  excurse_dat,
  # Excursion summary
  ...
  moduleServer(id, function(input, output, session) {
    ...
    shiny::observe({
      ...
      fraction_choices <- sort(unique(
        filtered_data1()$TADA.ResultSampleFractionText
      ))
    ...
    filtered_data1_1 <- shiny::reactive({
      ...
      dat2 <- filtered_data1() |>
        dplyr::filter(
          TADA.ResultSampleFractionText %in% input$fraction_box_select
        )
    ...
    shiny::observe({
      ...
      if (loc_select() %in% "MLid") {
        loc_choices <- sort(unique(
          filtered_data2()$TADA.MonitoringLocationIdentifier
        ))
      } else if (loc_select() %in% "AU") {
        loc_choices <- sort(unique(
          filtered_data2()$ATTAINS.AssessmentUnitIdentifier
        ))
      } else {
    ...
    filtered_data3 <- shiny::reactive({
      ...
      if (loc_select() %in% "MLid") {
        dat2 <- filtered_data2() |>
          dplyr::filter(
            TADA.MonitoringLocationIdentifier %in% input$loc_box_select
          )
      } else if (loc_select() %in% "AU") {
        dat2 <- filtered_data2() |>
          dplyr::filter(
            ATTAINS.AssessmentUnitIdentifier %in% input$loc_box_select
          )
      } else {
        dat2 <- filtered_data2()
      }
    ...
    shiny::observe({
      ...
      shinyjs::toggleState(id = "unique_box_select", condition = rv$unique_flag)
    ...
    shiny::observe({
      ...
      shinyjs::toggleState(id = "season_box_select", condition = rv$season_flag)
    ...
    filtered_data4 <- shiny::reactive({
      ...
      if (isTRUE(rv$season_flag)) {
        dat3 <- dat2 |> dplyr::filter(Season %in% input$season_box_select)
      } else {
        dat3 <- dat2
      }
    ...
    output$boxplot_view <- shiny::renderPlot({
      ...
      p <- ggplot2::ggplot() +
        ggplot2::geom_boxplot(
          data = filtered_data4(),
          ggplot2::aes(x = ATTAINS.UseName, y = TADA.ResultMeasureValue),
          color = 'gray30',
          outlier.shape = NA
        )
      ...
      if (loc_select() %in% "MLid") {
        rv$p_boxplot <- p +
          ...
          ggplot2::xlab('Uses') +
          ggplot2::ylab(paste0(
            unique(filtered_data4()$TADA.CharacteristicName),
            ' (',
        ...
        rv$p_boxplot <- p +
          ...
          ggplot2::xlab('Uses') +
          ggplot2::ylab(paste0(
            filtered_data4()$TADA.CharacteristicName,
            ' (',
            filtered_data4()$TADA.ResultMeasure.MeasureUnitCode,
            ')'
          )) +
    ...
    output$timeseries_view <- shiny::renderPlot({
      ...
      p <- ggplot2::ggplot(
        df,
        ggplot2::aes(x = ActivityStartDate, y = TADA.ResultMeasureValue)
      ) +
        ggplot2::geom_point(
        ...
        ggplot2::xlab("Time") +
        ggplot2::ylab(paste0(
          filtered_data4()$TADA.CharacteristicName[1],
          " (",
          filtered_data4()$TADA.ResultMeasure.MeasureUnitCode[1],
          ")"
        )) +
      ...
      plot_preprocess <- function(df_sub, value_col) {
        if (!value_col %in% names(df_sub)) {
          return(NULL)
        }
        df_valid <- df_sub[
          !is.na(df_sub[[value_col]]) & df_sub[[value_col]] > 0,
        ]
      ...
      plot_criteria(
        ...
        "MagnitudeValueUpper",
        "red",
        mag_upper$is_equation_based
      )
      plot_criteria(
        mag_lower$df_valid,
        "MagnitudeValueLower",
        "blue",
        mag_lower$is_equation_based
      )

      # Get the final equation-based flag
      is_equation_based_final <- mag_upper$is_equation_based |
        mag_lower$is_equation_based
      ...
      if (is_equation_based_final) {
        ...
        legend_points <- data.frame(
          ...
          Value = rep(
            min(
              df$TADA.ResultMeasureValue[df$TADA.ResultMeasureValue > 0],
              na.rm = TRUE
            ),
            6
          ),
    ...
    shiny::observe({
      shinyjs::toggleState(
        "download_boxplot",
        condition = !is.null(rv$p_boxplot)
      )
      shinyjs::toggleState(
        "download_timeseries",
        condition = !is.null(rv$p_timeseries)
      )
    })
