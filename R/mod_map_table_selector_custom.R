...
mod_map_table_selector_custom_ui <- function(id) {
  ...
  tagList(
    fluidRow(column(
      ...
      htmltools::p(
        "Select the sites on the map or in the table to conduct analysis. Use the checkbox to select all sites"
      ),
      htmltools::p(
        "If users select 'Custom Grouping' in the 'Custom Analyzed by the spatial unit' menu, selected site will be grouped for the analysis."
      ),
      htmltools::div(
    ...
    fluidRow(
      column(width = 6, leaflet::leafletOutput(ns("map_selector_custom"))),
      column(width = 6, DT::DTOutput(ns("table_selector_custom")))
    )
  )
}
...
mod_map_table_selector_custom_server <- function(id, tadat) {
  moduleServer(id, function(input, output, session) {
    ...
    shiny::observe({
      req(tadat$criteria_state_tribe, tadat$uses_select_re_custom)
      ...
      # Proxy
      map_selector_custom_proxy <- leaflet::leafletProxy(
        "map_selector_custom",
        session = session
      )
      table_selector_custom_proxy <- DT::dataTableProxy(
        "table_selector_custom",
        session = session
      )
        ...
        input$select_all_sites,
        {
          ...
          map_selector_custom_proxy |>
            leaflet::addCircleMarkers(
              ...
              layerId = paste0(
                tadat$site_AU_table_custom$TADA.MonitoringLocationIdentifier,
                "_new"
              ),
              radius = 9,
              stroke = TRUE,
      ...
      observeEvent(
        input$table_selector_custom_rows_selected,
        ignoreInit = TRUE,
        ignoreNULL = FALSE,
        {
      ...
      observeEvent(
        input$map_selector_custom_marker_click,
        {
          req(is.data.frame(tadat$site_AU_table_custom))
          req(nrow(tadat$site_AU_table_custom) > 0)
          ...
          if (stringr::str_detect(id, "_new$")) {
            ...
            if (length(new_selection) == 0) {
              ...
              map_selector_custom_proxy |>
                leaflet::clearGroup("highlighted_point")
            } else {
      ...
      observeEvent(
        c(tadat$criteria_state_tribe, tadat$uses_select_re_custom),
        {
    ...
    selected_monitoring_locations <- reactive({
      ...
      selected_rows <- tadat$site_AU_table_custom |> dplyr::slice(idx)
