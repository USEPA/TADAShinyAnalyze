testServer(mod_criteria_table_server, args = list(), {
  ns <- session$ns
...
testthat::test_that("mod_criteria_table_ui renders key controls", {
  ...
  html <- paste0(
    capture.output(htmltools::renderTags(ui)$html),
    collapse = "\n"
  )

  testthat::expect_true(grepl("2\\. Criteria Table", html))
  testthat::expect_true(grepl(
    "Select the method to generate a draft criteria and methods template",
    html
  ))
...
test_that("mod_criteria_table_server Option E upload path runs without error", {
  ...
  minimal_df <- data.frame(
    ...
    c4 = NA_character_,
    c5 = NA_character_,
    c6 = NA_character_, # >=6 cols
    stringsAsFactors = FALSE
  )
  writexl::write_xlsx(
    list(DefineCriteriaMethodology = minimal_df),
    path = tmp_xlsx
  )
...
test_that("mod_criteria_table_server Option E upload and summary works (mocked validators)", {
  ...
  testthat::with_mocked_bindings(
    runAllValidations = function(data) {
      list(overall_status = "Validation status: OK")
    },
