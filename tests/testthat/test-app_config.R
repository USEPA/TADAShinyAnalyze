test_that("app_sys returns package root and handles missing files", {
  ...
  skip_if_not(nzchar(cfg_path) && file.exists(cfg_path),
...
test_that("get_golem_config returns specific keys and falls back to default", {
  ...
  expect_identical(get_golem_config("DOES_NOT_EXIST", default = "fallback", file = tf), "fallback")
  
  # Different profile selection
  expect_equal(get_golem_config("MB_LIMIT", config = "production", file = tf), 456)
})
...
test_that("get_golem_config handles missing or unreadable config files", {
  ...
  expect_identical(get_golem_config("MB_LIMIT", default = 999, file = tf_missing), 999)
  ...
  expect_identical(get_golem_config("MB_LIMIT", default = 777, file = tf_bad), 777)
})